...
:script
:type tool
:desc Splits all selected meshes into unique meshes depending on their given face colour
"""
import pymxs
...
class MeshSplitter(object):
    ...
    def run(self, node):
        ...
        if(node_superclass == pymxs.runtime.GeometryClass):
            ...
            for i in range(1, temp_copy.getNumFaces() + 1):
                face_id = i

                pymxs.runtime.polyOp.setFaceSelection(temp_copy, [face_id])
                face_mat_id = temp_copy.getFaceColor(pymxs.runtime.name("VertexColor"))
            ...
            for mat_id in ids_grouped:
                ...
                split_name = temp_copy_name + "_color_" + str(mat_id.r) + "_" + str(mat_id.g) + "_" + str(mat_id.b)
